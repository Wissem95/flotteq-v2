...
export class DriverFactory {
  static create(data: Partial<Driver> = {}): Driver {
    ...
    driver.licenseNumber = data.licenseNumber || `LIC${Math.floor(Math.random() * 100000)}`;
    driver.licenseExpiryDate = data.licenseExpiryDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
    driver.phone = data.phone || `+33 6 ${Math.floor(Math.random() * 100000000)}`;
