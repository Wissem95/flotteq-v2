...
describe('DocumentOwnershipGuard', () => {
  ...
  describe('SUPER_ADMIN role', () => {
    it('should allow access to any document', async () => {
      ...
      await expect(
        guard.canActivate(createContext(user, 'DELETE', 'doc1')),
      ).resolves.toBe(true);
    });
  });
  ...
  describe('VIEWER role', () => {
    it('should allow GET requests', async () => {
      ...
      await expect(
        guard.canActivate(createContext(user, 'GET', 'doc1')),
      ).resolves.toBe(true);
    });
  ...
  describe('Document not found', () => {
    it('should throw NotFoundException', async () => {
      ...
      documentsService.findOne.mockRejectedValue(
        new NotFoundException('Document non trouvé'),
      );
