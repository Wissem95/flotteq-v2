...
export class AvailableSlotDto {
  ...
  @ApiProperty({ example: '16:00', description: 'End time of slot in HH:mm format' })
  endTime: string;

  @ApiProperty({ example: true, description: 'Is this slot available for booking?' })
  available: boolean;

  @ApiPropertyOptional({ example: 'Already booked', description: 'Reason if slot is not available' })
  reason?: string;
}

export class AvailableSlotsResponseDto {
  @ApiProperty({ example: '2025-10-20', description: 'Date of availability check' })
  date: string;
