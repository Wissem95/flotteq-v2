...
export class InitialSchema1759242722889 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."drivers_status_enum" AS ENUM('active', 'inactive', 'suspended', 'on_leave')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."vehicles_status_enum" AS ENUM('available', 'in_use', 'maintenance', 'out_of_service')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."maintenances_type_enum" AS ENUM('preventive', 'corrective', 'inspection', 'tire_change', 'oil_change')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."maintenances_status_enum" AS ENUM('scheduled', 'in_progress', 'completed', 'cancelled')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "vehicles" ADD CONSTRAINT "FK_206dc3b1eaf70992421f45433fa" FOREIGN KEY ("assigned_driver_id") REFERENCES "drivers"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE "maintenances" ADD CONSTRAINT "FK_d501088741618ca66074d85e411" FOREIGN KEY ("vehicle_id") REFERENCES "vehicles"("id") ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "maintenances" DROP CONSTRAINT "FK_d501088741618ca66074d85e411"`,
    );
    await queryRunner.query(
      `ALTER TABLE "vehicles" DROP CONSTRAINT "FK_206dc3b1eaf70992421f45433fa"`,
    );
