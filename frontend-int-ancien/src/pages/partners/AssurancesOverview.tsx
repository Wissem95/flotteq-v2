...
const AssurancesOverview: React.FC = () => {
  ...
  const loadAssurances = async () => {
    try {
      ...
      toast({
        title: 'Erreur',
        description: 'Impossible de charger les compagnies d\'assurance',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleToggleStatus = async (partnerId: number, isActive: boolean) => {
    try {
      ...
      toast({
        title: 'Succès',
        description: `Assurance ${!isActive ? 'activée' : 'désactivée'} avec succès`,
      });
      loadAssurances();
    } catch (error) {
      toast({
  ...
  const handleToggleVerification = async (partnerId: number, isVerified: boolean) => {
    try {
      ...
      toast({
        title: 'Succès',
        description: `Assurance ${!isVerified ? 'vérifiée' : 'non vérifiée'} avec succès`,
      });
      loadAssurances();
    } catch (error) {
      toast({
  ...
  const getStatusBadge = (partner: Partner) => {
    ...
    return <Badge variant="default" className="bg-green-600"><CheckCircle className="w-3 h-3 mr-1" />Partenaire</Badge>;
  };
  ...
  const formatPricing = (pricing?: Record<string, number>) => {
    ...
    const basePrice = pricing.assurance_base || pricing.assurance_essentielle;
    if (basePrice) {
      return `À partir de ${basePrice}€/mois`;
    }
    return 'Sur devis';
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gestion des compagnies d'assurance partenaires</p>
        </div>
          ...
          <div className="flex gap-4 flex-wrap">
            <div className="flex-1 min-w-[300px]">
              <div className="relative">
                ...
                <Input
                  placeholder="Rechercher une assurance (nom, ville, services)..."
                  className="pl-10"
                  value={searchTerm}
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="verified">Partenaires seulement</SelectItem>
              </SelectContent>
            </Select>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Compagnies d'Assurance ({partners.length})</CardTitle>
          <CardDescription>
            Gérez toutes vos compagnies d'assurance partenaires depuis cette interface
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <div className="text-gray-500 mb-4">
                {searchTerm || statusFilter !== 'all' 
                  ...
                  : 'Commencez par ajouter votre première compagnie d\'assurance partenaire'
                }
              </div>
              <Button>
...
export default AssurancesOverview;