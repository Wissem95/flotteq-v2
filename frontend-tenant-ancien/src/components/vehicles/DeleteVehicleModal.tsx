...
import { fetchVehicleById, deleteVehicle, Vehicle } from "@/services/vehicleService";
...
const DeleteVehicleModal: React.FC<DeleteVehicleModalProps> = ({
  ...
  const [vehicle, setVehicle] = useState<{ marque: string; modele: string; immatriculation: string } | null>(null);
  ...
  useEffect(() => {
    ...
    fetchVehicleById(String(vehicleId))
      .then((v) =>
        setVehicle({ marque: v.marque, modele: v.modele, immatriculation: v.immatriculation })
      )
      .catch(() => {
        setVehicle(null);
